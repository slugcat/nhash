...
public class Defaults {
        ...
        @Override public Set<Entry<K,V>> entrySet() {
            return new AbstractSet<Entry<K,V>>() {
                ...
                @Override public Iterator<Entry<K,V>> iterator() {
                    final Iterator<Entry<K,V>> source = map.entrySet().iterator();
                    return new Iterator<Entry<K,V>>() {
                       public boolean hasNext() { return source.hasNext(); }
                       public Entry<K,V> next() { return source.next(); }
    ...
    public static class ImplementsConcurrentMap<K,V> extends ExtendsAbstractMap<ConcurrentMap<K,V>, K, V> implements ConcurrentMap<K,V> {
        ...
        @Override public V replace(K k, V v) {
            return map.replace(Objects.requireNonNull(k), Objects.requireNonNull(v));
        }

        @Override public boolean replace(K k, V v, V vv) {
            return map.replace(Objects.requireNonNull(k),
                               Objects.requireNonNull(v),
                               Objects.requireNonNull(vv));
        }

        @Override public boolean remove(Object k, Object v) {
            return map.remove(Objects.requireNonNull(k), Objects.requireNonNull(v));
        }

        @Override public V putIfAbsent(K k, V v) {
            return map.putIfAbsent(Objects.requireNonNull(k), Objects.requireNonNull(v));
        }
    }
}
